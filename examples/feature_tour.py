...
def tour_preview():
    ...
    for _ in range(10):
        try:
            ...
            time.sleep(1.5)
    if got == 200:
        ok("preview", f"fetched {url} from the public internet (HTTP 200)")
    else:
        fail("preview", f"could not reach {url} (last status: {got})")
    delete_sandbox(sid)
...
def tour_browser():
    ...
    if st == 200 and b.get("vnc") and b.get("cdp"):
        ok("browser", "desktop up with VNC + CDP endpoints")
        info(f"watch it live:  {b['vnc']}")
        info(f"drive it (Playwright connectOverCDP):  {b['cdp']}")
    else:
